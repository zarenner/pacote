...
function Manifest (pkg, fromTarball) {
  ...
  this._sha256sum = pkg._sha256sum || fromTarball._sha256sum
...
function tarballedProps (pkg, spec, opts, cb) {
  ...
  var needsShasum = !pkg._shasum && !pkg._sha256sum
  if (!needsShrinkwrap && !needsBin && !needsShasum) {
    ...
    if (needsShasum) {
      ...
      shaStream.on('digest', function (d) {
        ...
        } else if (opts.hashAlgorithm === 'sha256') {
          extraProps._sha256sum = d
        } else {
          cb(new Error('only sha1 and sha256 hashes are supported'))
        }
      })
    } else {
      shaStream = through()
