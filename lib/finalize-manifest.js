...
function finalizeManifest (pkg, spec, opts) {
  ...
  return ((opts.cache && key && !opts.preferOnline) ? cache.get.info(opts.cache, key, opts).then(res => {
    ...
    return opts.verifySignatures ? checkSigs(manifest) : manifest
  })
}
...
function checkSigs (manifest) {
  if (manifest._integrity) {
    const integrity = ssri.parse(manifest._integrity)
    const algo = integrity.pickAlgorithm()
    const hashes = integrity[algo]
    return BB.any((hashes || []).map(hash => {
      const sigs = hash.options.map(opt => {
        return opt.match(/sig-([^:]+):(.*)/)
      }).filter(x => x)
      if (!sigs.length) {
        // No signatures!
        return BB.resolve(manifest)
      } else if (sigs && !verifySignature.hasKeybase) {
        const err = new Error(`${manifest.name}@{manifest.version} has signatures attached but client cannot verify them`)
        err.code = 'ENOKEYBASE'
        throw err
      }
      return BB.any(sigs.map(sig => {
        const decodedSig = Buffer(sig[2], 'base64').toString('utf8')
        return verifySignature(sig[1], decodedSig).then(msg => {
          return msg === `${hash.algorithm}-${hash.digest}`
        }, err => {
          if (err.code === 2) {
            err.code = 'EBADSIG'
          }
      ...
      if (verified) {
        ...
        const err = new Error(`Signature verification failed for ${manifest.name}@${manifest.version} while checking signed integrity hash ${manifest._integrity}`)
        err.code = 'EBADSIG'
