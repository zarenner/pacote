...
function get (uri, registry, opts) {
  ...
  return pinflight(key, () => {
    return (
      ...
      if (entry && !isStale(entry.metadata, opts)) {
        return cache.get.byDigest(opts.cache, entry.digest, opts)
      } else {
        return registryFetch(key, uri, registry, entry && entry.metadata, opts).then(data => {
          if (data) {
            return data
          } else {
            return cache.get.byDigest(opts.cache, entry.digest, opts)
...
function registryFetch (key, uri, registry, meta, opts) {
  const authKey = opts[registryKey(registry) + ':_authToken']
  return BB.fromNode(cb => {
    client(opts).get(uri, {
      etag: meta && meta.etag,
      lastModified: meta && meta.lastModified,
      follow: opts.follow,
      auth: authKey && { token: authKey },
      timeout: opts.timeout
      ...
      if (res.statusCode === 304) {
        ...
        if (!data && res.headers['content-encoding'] === 'gzip') {
          raw = zlib.gunzipSync(raw)
        }
        if (opts.cache) {
          const localopt = Object.create(opts)
          localopt.metadata = {
            etag: res.headers['etag'],
            lastModified: res.headers['last-modified'],
            cacheControl: res.headers['cache-conrol'],
          ...
          cache.put(opts.cache, key, raw, localopt).then(() => {
            cb(null, raw)
          }, cb)
        } else {
          cb(null, raw)
